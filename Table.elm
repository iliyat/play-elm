...
createState : String -> Bool -> State
createState header bool =
    State header bool
...
defaultCustomizations =
    { tableAttrs = [ Attr.class "elm-table" ]
    , caption = Nothing
    , thead = simpleThead
    , tfoot = Nothing
    , tbodyAttrs = []
...
simpleThead headers =
    ...
    in
        HtmlDetails [] (List.map simpleTheadHelp indexed)


simpleTheadHelp : ( Int, ( String, Status, Attribute msg ) ) -> Html msg
            ...
            case status of
                ...
                Reversible Nothing ->
                    case isFirstIndex of
                        True ->
                            [ Html.span [ Attr.class "arrow-button" ]
                                [ Html.text name
                ...
                Reversible (Just isReversed) ->
                    case isFirstIndex of
                        True ->
                            [ Html.span [ Attr.class "arrow-button bold" ]
                                [ Html.text name
                                , if isReversed then
...
-}
view : Config data msg -> State -> List data -> Int -> Html msg
view (Config { toId, toMsg, columns, customizations }) state data limit =
    let
        sortedData =
            List.take limit <| sort state columns data
